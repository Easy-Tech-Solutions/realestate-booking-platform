...
// No ML risk model is wired up yet — every flag here comes from the
// rule-based detectors in trustsafety/detection.py. This badge marks where a
// real score will show once one is integrated.
function AiScoreBadge({ score }: { score: number | null }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
      <Sparkles className="h-3 w-3" /> {score === null ? 'AI risk score: not yet configured' : `AI risk score: ${score}`}
    </span>
  );
}
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Blocks registration and account creation from a matching device. The frontend does not yet
          compute and send a device fingerprint on every request — this list is enforced wherever the
          <code className="mx-1">X-Device-Fingerprint</code> header is present.
        </p>
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <section className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          accounts) — there is no ML risk model wired in yet.
        </p>
        {loading ? (
          <p className="text-sm text-muted-foreground">Loading…</p>
        ) : error ? (
