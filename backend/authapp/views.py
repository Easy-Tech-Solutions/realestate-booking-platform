...
def register(request):
    ...
    age_confirmed = request.data.get("age_confirmed")
    ...
    # The signup form self-certifies this via a checkbox rather than
    # collecting an exact birthdate.
    if str(age_confirmed).strip().lower() not in ('true', '1', 'yes', 'on'):
        return Response({"error": "You must confirm you are at least 18 years old to register."}, status=status.HTTP_400_BAD_REQUEST)
